...
descriptions = [] #these are for red globion gene names
...
fig, ax = plt.subplots()
ax.scatter(k562_model_predictions, k562_observations, color ='blue', s=0.1, alpha = 1)
ax.set_xlabel("Predicted K562 expression level \n10-fold cross validated")
ax.set_ylabel("K562 expression level (log10)")
line_xs = np.linspace(max(min(k562_model_predictions),min(k562_observations)), min(max(k562_model_predictions),max(k562_observations)))
line_ys = 0 + 1 * line_xs
